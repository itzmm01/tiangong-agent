...
# Name: monitor-elasticsearch-indices-docs-deleted.sh
# Description: a script to monitor elasticsearch indices_docs_deleted.
################################################################
# name of script
baseName=$(basename $0)
#desc: print log
...
#desc: get elasticsearch indices_docs_deleted
elasticsearch_indices_docs_deleted()
{
    ...
    count=$(curl -s http://${ip}:${port}/_cluster/stats --header "Authorization: Basic ${auth}"|jq '.indices.docs.deleted')
    echo "$count"
}
