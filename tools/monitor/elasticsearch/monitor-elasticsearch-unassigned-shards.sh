...
# Name: monitor-elasticsearch-unassigned-shards.sh
# Description: a script to monitor elasticsearch unassigned_shards.
################################################################
# name of script
baseName=$(basename $0)
#desc: print log
...
#desc: get elasticsearch unassigned_shards
elasticsearch_unassigned_shards()
{
    ...
    shards=$(curl -s http://${ip}:${port}/_cluster/health  --header "Authorization: Basic ${auth}" |jq '.unassigned_shards')
    echo "$shards"
}
