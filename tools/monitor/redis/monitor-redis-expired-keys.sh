...
# Name: monitor-redis-expired-keys.sh
# Description: a script to monitor redis expired keys. 
################################################################
# name of script
baseName=$(basename $0)
#desc: print log
...
#desc: get redis expired keys
redis_expired_keys()
{
    check_command redis-cli
    check_input "$@"
    keys=$(redis-cli -h $1 -p $2 -a $3 info|grep "expired_keys:")
    if [ ! -n  "$keys" ];then
