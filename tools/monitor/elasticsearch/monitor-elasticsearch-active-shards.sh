...
# Name: monitor-elasticsearch-active-shards.sh
# Description: a script to monitor elasticsearch active_shards.
################################################################
# name of script
baseName=$(basename $0)
#desc: print log
...
#desc: get elasticsearch active_shards
elasticsearch_active_shards()
{
    ...
    shards=$(curl -s http://${ip}:${port}/_cluster/health --header "Authorization: Basic ${auth}" |jq '.active_shards')
    echo "$shards"
}
