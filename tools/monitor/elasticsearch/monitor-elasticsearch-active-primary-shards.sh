...
# Name: monitor-elasticsearch-active-primary-shards.sh
# Description: a script to monitor elasticsearch active_primary_shards.
################################################################
# name of script
baseName=$(basename $0)
#desc: print log
...
#desc: get elasticsearch active_primary_shards
elasticsearch_active_primary_shards()
{
    ...
    shards=$(curl -s http://${ip}:${port}/_cluster/health --header "Authorization: Basic ${auth}" |jq '.active_primary_shards')
    echo "$shards"
}
