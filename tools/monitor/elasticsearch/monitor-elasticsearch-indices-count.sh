...
# Name: monitor-elasticsearch-indices-count.sh
# Description: a script to monitor elasticsearch indices_count.
################################################################
# name of script
baseName=$(basename $0)
#desc: print log
...
#desc: get elasticsearch indices_count
elasticsearch_indices_count()
{
    ...
    count=$(curl -s http://${ip}:${port}/_cluster/stats --header "Authorization: Basic ${auth}"|jq '.indices.count')
    echo "$count"
}
