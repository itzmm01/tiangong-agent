...
function print_usage() {
  ...
  print_log "INFO" "  $baseName 127.0.0.1 8888 UDP 10 100M 2048 loss 0.1 gt"
  print_log "INFO" "  $baseName 127.0.0.1 8888 TCP 10 100M 2048 loss 0.1 gt 0.01"
}
# desc: check input
...
main() {
  ...
  local ratio=1
  if [ $# -eq 10 ]; then
    ratio=$(awk -v x=1 -v y="${10}" 'BEGIN{printf "%.3f",x+y}')
  fi
