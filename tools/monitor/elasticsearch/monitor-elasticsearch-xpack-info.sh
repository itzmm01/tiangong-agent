...
# Name: monitor-elasticsearch-xpack-info.sh
# Description: a script to monitor elasticsearch xpack info.
################################################################
# name of script
baseName=$(basename $0)
#desc: print log
...
#desc: get elasticsearch xpack info
elasticsearch_xpack_info()
{
    check_input "$@"
    ip=$1
    port=$2
    ...
    info=$(curl -s http://${ip}:${port}/_xpack --header "Authorization: Basic ${auth}")
    echo "$info"
}
