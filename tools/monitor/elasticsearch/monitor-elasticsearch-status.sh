...
# Name: monitor-elasticsearch-status.sh
# Description: a script to monitor elasticsearch status.
################################################################
# name of script
baseName=$(basename $0)
#desc: print log
...
elasticsearch_status()
{
    ...
    status=$(curl -s http://${ip}:${port}/_cluster/health --header "Authorization: Basic ${auth}" |jq '.status')
    echo "$status"
}
