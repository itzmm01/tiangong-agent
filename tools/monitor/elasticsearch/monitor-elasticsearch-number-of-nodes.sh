...
# Name: monitor-elasticsearch-number-of-nodes.sh
# Description: a script to monitor elasticsearch number_of_nodes.
################################################################
# name of script
baseName=$(basename $0)
#desc: print log
...
#desc: get elasticsearch number_of_nodes
elasticsearch_number_of_nodes()
{
    ...
    total=$(curl -s http://${ip}:${port}/_cluster/health --header "Authorization: Basic ${auth}" |jq '.number_of_nodes')
    echo "$total"
}
