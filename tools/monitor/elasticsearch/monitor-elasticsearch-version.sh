...
# Name: monitor-elasticsearch-version.sh
# Description: a script to monitor elasticsearch version.
################################################################
# name of script
baseName=$(basename $0)
#desc: print log
...
#desc: get elasticsearch version
elasticsearch_info()
{
    check_input "$@"
    ip=$1
    ...
    info=$(curl -s http://${ip}:${port} --header "Authorization: Basic ${auth}" |jq '.version.number')
    echo "$info"
}
