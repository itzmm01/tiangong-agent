...
# Name: monitor-elasticsearch-indices-segments-count.sh
# Description: a script to monitor elasticsearch indices_segments_count.
################################################################
# name of script
baseName=$(basename $0)
#desc: print log
...
#desc: get elasticsearch indices_segments_count
elasticsearch_indices_segments_count()
{
    ...
    count=$(curl -s http://${ip}:${port}/_cluster/stats --header "Authorization: Basic ${auth}"|jq '.indices.segments.count')
    echo "$count"
}
