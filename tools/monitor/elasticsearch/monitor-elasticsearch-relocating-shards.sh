...
# Name: monitor-elasticsearch-relocating-shards.sh
# Description: a script to monitor elasticsearch relocating_shards.
################################################################
# name of script
baseName=$(basename $0)
#desc: print log
...
#desc: get elasticsearch relocating_shards
elasticsearch_relocating_shards()
{
    ...
    shards=$(curl -s http://${ip}:${port}/_cluster/health --header "Authorization: Basic ${auth}" |jq '.relocating_shards')
    echo "$shards"
}
